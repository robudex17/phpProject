var mytestexport = document.getElementById('export')
   mytestexport.addEventListener('click', salesCallDetailsExport )

document.getElementById('clickdate').addEventListener('submit', salesCallDetails)

function salesCallDetails() {
   ...
   document.getElementById('agentusername').innerHTML = username[1];
    ...
    xhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        ...
        salesCallDetailsTable(this.responseText,tbody);
        
      }
    };
   
      var apiquery = `api/sales_call_details.php?${querystring}`;
...
function salesCallDetailsExport() {
    ...
    let url = `http://103.5.6.2/sbtph_qc_pub/api/sales_call_details_export.php?${querystring}`
...
function salesCallDetailsTable(res,tbody) {
       ...
       // listenBtn.dataset.dismiss = "modal";
        ...
        updateBtn.addEventListener('click', function(e){
           ...
           data.comment = getExistingComment.value
             ...
             fetch('http://103.5.6.2/sbtph_qc_pub/api/putsales_comment.php', {method:'post', body:JSON.stringify(data)})
             .then(response => {
                 return response.json()
             }).then(res => {
                 ...
                 let url = `http://103.5.6.2/sbtph_qc_pub/api/get_single.php?${params}`
                 return fetch(url)
             }).then(response => 
                {return response.json()}
             ).then(data => {
