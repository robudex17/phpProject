...
class Sales {
   ...
   //create database connection  when this class instantiated
    ...
    public function sales_summary($getdate){
     ...
     public function sales_summary_export($getdate){
        ...
        if($stmnt->execute()){
             ...
             $sales_agent_summary_template_json = file_get_contents("/var/www/html/sbtph_qc/json/sales_call_summary.json");
            //make an object 
    ...
    public function salesTransfer($extension){

      //  build query
    	$query = "SELECT * FROM " . $this->salesteam_callsummary_table . " WHERE Caller=?";
    	// $query  = "SELECT StartTimeStamp,EndTimeStamp FROM ".$this->collectionteam_callsummary." WHERE getDate='2019-09-13' AND CallStatus='ANSWER'  AND Caller ='6340'";
    	// //prepare the query
    	 $stmnt = $this->conn->prepare($query);
    	...
    	if ($num != 0 ){
			...
			while($row = $stmnt->fetch(PDO::FETCH_ASSOC)){
				$this->doTransfer($row['StartTimeStamp'],$row['EndTimeStamp'],$row['Duration'],$row['CallStatus'],$row['Caller'],$row['CalledNumber'],$row['getDate'],$row['recording_link'],$row['comment']);
				 $agent = array(
                    "StartTimeStamp" => $row['StartTimeStamp'],
    ...
    public function createAgent() {
     ...
     public function doTransfer($startimestamp,$endtimestamp,$duration,$callstatus,$caller,$callednumber,$getdate,$recordlink,$comment) {
     // public function doTransfer() {
        // //create query

        // $query = " INSERT INTO " . $this->transfer . " SET StartTimeStamp = :StartTimeStamp, EndTimeStamp = :EndTimeStamp, Duration  = :Duration, CallStatus = :CallStatus, Caller = :Caller, CalledNumber = :CalledNumber, recording_link = :recording_link, getDate =:getDate, comment =:comment"  ;
        ...
        // $stmnt->bindParam(":StartTimeStamp", $startimestamp);
        // $stmnt->bindParam(":EndTimeStamp", $endtimestamp);
        // $stmnt->bindParam(":Duration", $duration);
        // $stmnt->bindParam(":CallStatus", $callStatus);
        // $stmnt->bindParam(":Caller", $caller );
        // $stmnt->bindParam(":CalledNumber", $callednumber);
        // $stmnt->bindParam(":recording_link", $recordlink);
        // $stmnt->bindParam(":getDate", $getdate);
        ...
        $sql = "INSERT INTO collectionteam_callsummary (StartTimeStamp, EndTimeStamp, Duration,CallStatus,Caller,CalledNumber,getDate,recording_link,comment)
        VALUES ('$startimestamp', '$endtimestamp', '$duration', '$callstatus', '$caller', '$callednumber', '$getdate', '$recordLink', '$comment')";
