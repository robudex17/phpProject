...
function getActiveAgents(){
   ...
   fetch('http://192.168.70.250/sbtph-dev/api/active.php').then(response => {
...
function getInactiveAgents(){
   getLoginUser()
  fetch('http://192.168.70.250/sbtph-dev/api/inactive.php').then(response => {
    return response = response.json();
  }).then(data =>{
	...
	//Creating Modal Form
	       ...
	       // listenBtn.dataset.dismiss = "modal";
	        ...
	        listenBtn.addEventListener('click', function(e){
	            ...
	            fetch(`http://192.168.70.250/sbtph-dev/utils/chanspy.php?querystring=${querystring}`).then(response =>{
	             return response = response.json()
	            }).then(data => 
	              console.log(data)
	            ).catch(err =>{
